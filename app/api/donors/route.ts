...
export async function POST(req: Request) {
  ...
  // Step 3: Add each blood bag to blood_groups table 🩸
  const expireTime = new Date();
  expireTime.setMonth(expireTime.getMonth() + 3); // 3 months validity

  // Create 'quantity' number of blood group entries
  const bloodBags = Array.from({ length: quantity }).map(() => ({
    blood_bank_id: admin.blood_bank_id,
    type: blood_type,
  ...
  return NextResponse.json(
    { message: `${quantity} blood bag(s) recorded successfully.` },
    { status: 201 }
  );
}
